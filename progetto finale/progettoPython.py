...
class CardViewerApp:
    ...
    def build_query(self):
        """Costruisce la query per Scryfall con AND espliciti tra i filtri"""
        parts = []
        ...
        if oracle:
            parole = oracle.split()
            # Mettiamo AND esplicito fra le parole oracle
            oracle_query = " AND ".join(f"oracle:{p}" for p in parole)
            # Inseriamo la parte oracle tutta fra parentesi
            parts.append(f"({oracle_query})")

        # Uniamo tutte le parti separate da AND esplicito per evitare ambiguità
    ...
    def search_cards(self, query):
        ...
        try:
            ...
            # Prende solo le prime 30 carte per non sovraccaricare la GUI
            cards = data["data"][:50]
            # Chiama la funzione per mostrare le carte nella GUI nel thread principale
    ...
    def load_image(self, card, label):
        ...
        try:
            ...
            # Aggiorna la label con l'immagine (e rimuove il testo "Caricamento...") nel thread principale
            self.root.after(0, lambda: label.config(image=tk_image, text=""))

        except Exception:
            # In caso di errore nel download o caricamento mostra messaggio nel thread principale
